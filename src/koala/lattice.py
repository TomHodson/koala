...
class Lattice(object):
    ...
    def __init__(
        ...
        edge_vectors = (
            vertices[edge_indices][:, 1] - vertices[edge_indices][:, 0] + edge_crossing
        )
    ...
    def __getstate__(self):
        ...
        def check_fits(array, dtype):
            assert (
                np.iinfo(dtype).min <= np.min(array)
                and np.max(array) <= np.iinfo(dtype).max
            )
...
def _sorted_vertex_adjacent_edges(vertex_positions, edge_indices, edge_vectors):
    ...
    for index in range(vertex_positions.shape[0]):
        v_edges = np.nonzero(
            (edge_indices[:, 0] == index) + (edge_indices[:, 1] == index)
        )[0]
...
def _find_plaquette(starting_edge: int, starting_direction: int, l: Lattice):
    ...
    while True:
        ...
        current_vertex = edge_indices[current_edge][
            np.where(np.roll(edge_indices[current_edge], 1) == current_vertex)[0][0]
        ]
        ...
        current_direction = (
            1
            if np.where(edge_indices[current_edge] == current_vertex)[0][0] == 0
            else -1
        )
    ...
    plaquette_crossings = (
        plaquette_directions[:, None] * l.edges.crossing[plaquette_edges]
    )
