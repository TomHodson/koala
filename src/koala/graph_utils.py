...
def vertex_neighbours(lattice, vertex_i):
    ...
    start_or_end = (edges != vertex_i)[
        :, 1
    ]  # this is true if vertex_i starts the edge and false if it ends it
    vertex_indices = np.take_along_axis(
        edges, start_or_end[:, None].astype(int), axis=1
    ).flatten()  # this gets the index of the other end of each edge
...
def clockwise_about(vertex_i: int, g: Lattice) -> np.ndarray:
    ...
    angles = np.where(
        angles > 0, angles, 2 * np.pi + angles
...
def get_edge_vectors(vertex_i: int, edge_indices: np.ndarray, l: Lattice) -> np.ndarray:
    ...
    other_vertex_indices = np.take_along_axis(
        edges, start_or_end[:, None].astype(int), axis=1
    ).squeeze()  # this gets the index of the other end of each edge
    offset_sign = 2 * start_or_end - 1  # now it's +/-1

    # get the vectors along the edges
    return (
        l.vertices.positions[other_vertex_indices]
        - l.vertices.positions[vertex_i][None, :]
        + offset_sign[:, None] * l.edges.crossing[edge_indices]
    )
...
def adjacent_plaquettes(l: Lattice, p_index: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    neighbouring_plaquettes = np.take_along_axis(
        neighbouring_plaquettes, other_index, axis=1
    ).squeeze(axis=-1)
...
def closest_vertex(lattice, point):
    "Return the index of the vertex in the lattice closest to the given point"
    distances = np.linalg.norm(
        lattice.vertices.positions - np.array(point)[None, :], ord=2, axis=-1
    )
    return np.argmin(distances)


def closest_plaquette(lattice, point):
    "Return the index of the plaquette in the lattice with center closest to the given point"
